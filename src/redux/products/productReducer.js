 ...
 switch (action.type) {
  ...
  case SORT_PRODUCTS:
   return {
    ...
    allProducts: sortProducts(action.payload.sortBy, [...state.allProducts]),
   };

  case SEARCH_PRODUCTS:
   return {
    ...state,
    products: searchProducts(action.payload.searchTerm, [...state.allProducts]),
   };
  ...
  case REMOVE_ONE_PRODUCT:
   return {
    ...state,
    selectedProducts: [
     ...state.selectedProducts.filter((item) => item.id !== action.payload),
    ],
   };
